...
/**
 * Siri-style GLSL shaders rendered on a raw WebGL canvas (no external library:
 * a single fullscreen triangle drives a fragment shader).
 *
 * - `"wave"`        — the iOS voice waveform (chromatic, frequency-reactive).
 * - `"fluid-dots"`  — six metaball dots that merge, scatter and gather.
 *
 * The shaders are ported from the original `siriWaveCore` / `siriFluidDotsCore`
 * sources; the only change is the final alpha, derived from luminance so the
 * black background is transparent and the orb blends onto either theme.
 */
...
export function SiriWave({
  ...
  React.useEffect(() => {
    ...
    // Blend the luminance-keyed alpha over whatever sits behind the canvas.
    gl.enable(gl.BLEND);
